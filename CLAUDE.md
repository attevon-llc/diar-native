...
wrapped by our crates and served as the `diar-native` sidecar. **Released: 0.3.1** (crates and
image tag unified at that number; see `CHANGELOG.md`). 0.3.1 was built and published TWICE —
once at release, and again after `scripts/bootstrap_vendor_speakrs.sh` was repointed at a real
fork commit. Source identical (the patch regenerated byte-identically), `/diarize` output
byte-identical, digests different. `docs/DEPLOYMENT.md` carries the republished set; the
`v0.3.1` tag was moved onto `main` so tag, source and published digests agree.
...
is repointed — which is a change made in transcribe-app, not here. **As of 0.3.1 the live sidecar
is STILL a 0.2.0 build** — measured, not assumed: its `/healthz` returns the bare string `ok`
(0.3.x returns a JSON object) and `/readyz` 404s. That two-way check is the cheapest version
oracle available; use it before believing any test result about "the new binary".
transcribe-app PR #696 repoints the digest to
`sha256:83a709be94d0ca06441fa10aea0680f53b03cc10eb3ce11c4eeb84478400567d`, and nothing ships
until the shared backend image is rebuilt from it.
