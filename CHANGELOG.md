...
accuracy gates are identical to 0.3.0.

### Added

- **Deploy without cloning the repo.** Two files and one command:
  ...
  `docker compose up` alone provisions the models and then serves — a provisioning service runs
  to completion and the server gates on it with `service_completed_successfully`. Provisioning
  is idempotent, so every later `up` skips it without a token or a network call. Verified from
  an empty directory **outside any clone**: 484 MB provisioned, real diarization, and a **4.1 s**
  second start with the token blanked to prove it was not used.

- **`scripts/release.sh`** — releases are built, scanned and published locally. Hosted runners
  give ~14 GB of disk and a release is five images across two architectures, the CUDA one alone
  ~3 GB. `.github/workflows/release.yml` is removed; it had never completed a release (the
  `v0.3.0` run stopped at its credentials gate in 12 s, and every 0.3.0 image was pushed by
  hand). The script refuses a dirty tree, refuses if `vendor/speakrs` has drifted from the
  committed patch, asserts each image's architecture matches its tag name, asserts the binary
  reports the version being released, asserts the image is not root, fails on any HIGH/CRITICAL
  trivy finding, and verifies published digests **against the registry** rather than the local
  daemon. `--push` is explicit: a tag someone has already pulled cannot be un-pulled.

- **`start.sh` pulls published images by default**, selecting per host (amd64+GPU, amd64 CPU,
  arm64), and refuses rather than emulating if a host is pinned to the wrong architecture.
  `--build` is the contributor path.

- **`davidamacey/diar-native:<ver>-provision` and `-provision-arm64` are published.** The
  serving images contain no Python, so `provision-models` against one exits **6**. The
  documented install would have failed at its first command without them.
...
- **A clean clone could not be built.** `scripts/bootstrap_vendor_speakrs.sh` never applied
  `patches/0001-*.patch`, *and* pinned the wrong base — the fork's master `725cc4d`, which
  already contains the 0.2.0 patch set, when the patch is generated by `git diff HEAD` against
  upstream `b0756b1` and applies only there. Two contradictory reproduction stories for one
  tree. Harmless until the `SPEAKRS_FBANK_POOL` fix made `diar-core` require a patched speakrs
  for the first time, after which every container build and every CI run failed with
  `error[E0560]: struct RuntimeConfig has no field named fbank_pool`. It was invisible locally,
  because a developer's working tree already carries the patch as an uncommitted diff — the
  machine that makes a vendored change is the one machine that cannot see a broken bootstrap.
  Bootstrap now checks out `b0756b1`, applies the patch, and fails loudly if it does not apply.

- **`SPEAKRS_FBANK_POOL` works, and `DiarEngine::load` no longer mutates the environment
  (issue #3).** The pool size travels to speakrs through `RuntimeConfig` instead of an
  `env::set_var` round-trip. The knob had been documented as operator-settable while the
  unconditional `set_var` silently overwrote whatever was set — measured: `=3` was ignored
  before and is honoured now, with mode defaults unchanged (CUDA 8, CPU 1). The `SAFETY`
  premise it rested on was **already false**: `EngineRegistry::load` calls `DiarEngine::load`
  once per device, so `DIAR_DEVICES=cuda,cpu` ran the second `setenv` while the first engine's
  ORT threads were live, and speakrs reads two other knobs on the *request* path. Unblocks lazy
  engine loading, worth ~620 MB RSS. RESULTS §7.50.

- **Two docs pointed at a build path that no longer exists** — `docker-compose.prod.yml` and
  `start.sh` still named the deleted workflow.

### Changed

- **Containers run as a non-root user, uid 10001 (issue #7).** Serving mounts models `:ro` and
  writes nothing; provisioning runs as the *invoking* user, so the models land owned by the
  operator and no `chown` is ever needed. Zero bytes added — the CPU image is unchanged in size.
  Trivy 0.67.1: **0 HIGH / 0 CRITICAL** on all images, with the previously-released root image
  scanned as a control.

- **`DIAR_ORT_OPT_LEVEL` is a floor, not an override.** On aarch64 it can lower the gender
  session's optimization level but no longer raise it above `Level1`, where the fp16 model
  stops loading. An operator tuning the diarization graphs will no longer silently lose speaker
  gender. x86_64 is unaffected.

- **README leads with deployment**, not with the reference manual: run it, use it, develop it.

### Known

- **`ubuntu 26.04` changes diarization output on arm64** (exclusive segments 8 → 10 on one
  clip; amd64 unaffected, every ONNX numeric stage byte-identical). The base is pinned to
  `24.04`, so published images are on the known-good one. Whether that difference is a
  regression, an improvement or immaterial is **unscored** — issue #18.
